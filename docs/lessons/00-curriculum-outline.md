...
- **Reverse-engineer first**: Understand every piece of Xun before learning new things
...
The goal: be able to explain every line of Xun to a FAANG interviewer and justify every architectural decision.

### Section 01: Electron Architecture

How Xun works as a desktop app. The process model, IPC, and why Electron is built this way.
...
| 02 | Xun's Main Process Deep Dive | `[ ]` | Walk through `src/main/index.ts` line by line. App lifecycle, BrowserWindow creation, menu setup. |
| 03 | The Preload Bridge | `[ ]` | `src/preload/index.ts` - contextBridge, contextIsolation, why this security layer exists. |
| 04 | IPC Communication Patterns | `[ ]` | invoke/handle vs send/on. Map every IPC channel in Xun. Understand the data flow. |
| 05 | Xun's IPC Handlers | `[ ]` | Walk through `src/main/ipc/` - file operations, vault management, blog publishing channels. |
...
How the UI works. React's mental model, Xun's component tree, and state management.

| # | Lesson | Status | Description |
|---|--------|--------|-------------|
| 01 | React's Mental Model | `[ ]` | Rendering as snapshots. The component tree. Virtual DOM reconciliation. Why React works this way. |
| 02 | Xun's Component Tree | `[ ]` | Map App.tsx -> every component. Parent-child relationships. Props flow. |
| 03 | Hooks in Xun | `[ ]` | Every useState, useEffect, useRef, useCallback in the codebase. Why each one. Common mistakes. |
...
CodeMirror 6 internals and how Xun uses it for markdown editing.

| # | Lesson | Status | Description |
|---|--------|--------|-------------|
| 01 | CodeMirror 6 Architecture | `[ ]` | State vs View. Extensions. Transactions. The functional design. |
| 02 | Xun's Editor Setup | `[ ]` | Extension chain. Language support. Theme. Keybindings. |
...
The most complex feature in Xun - understand it completely.

| # | Lesson | Status | Description |
|---|--------|--------|-------------|
| 01 | Publishing Architecture | `[ ]` | `src/main/publish/` and `src/main/cms/`. The full data flow from editor to deployed blog. |
| 02 | GitHub API Integration | `[ ]` | How Xun pushes content to GitHub repos. Authentication. File creation/update. |
...
| 03 | Xun's Test Suite | `[ ]` | Walk through every test. What's covered, what's not. |
...
| 08 | System Design Practice | `[ ]` | Design a note-taking app (like Xun!) at scale. Design a CMS. Design a real-time editor. |
...
| # | Lesson | Status | Description |
|---|--------|--------|-------------|
| 01 | SOLID Principles in Practice | `[ ]` | Each principle with TypeScript/React examples from Xun. |
...
- **Xun Codebase** - The best learning tool is the app you built
...
### Technical Skills
- [ ] Can explain every piece of Xun's architecture to an interviewer
- [ ] Can solve Medium LeetCode problems in 25 minutes
- [ ] Can solve Hard LeetCode problems in 40 minutes (for some patterns)
- [ ] Can design a system like Xun at web scale (multi-tenant, millions of users)
