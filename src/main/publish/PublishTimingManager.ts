...
export class PublishTimingManager {
  ...
  constructor(vaultPath: string) {
    ...
    this.timingFilePath = path.join(vaultPath, '.xin', 'publish-times.json');
  }

  private async ensureXinFolder(): Promise<void> {
    const xinFolder = path.join(this.vaultPath, '.xin');
    try {
      await fs.mkdir(xinFolder, { recursive: true });
    } catch {
