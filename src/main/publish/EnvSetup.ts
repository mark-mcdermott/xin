 ...
 *   XUN_BLOG_1_NAME, XUN_BLOG_1_GITHUB_REPO, etc.
 *   XUN_BLOG_2_NAME, XUN_BLOG_2_GITHUB_REPO, etc.
 */
...
/**
 * Read and parse a .env file, filtering for XUN_BLOG_<N>_* keys,
...
export async function parseEnvFile(
  ...
  for (const line of content.split('\n')) {
    ...
    const match = key.match(/^XUN_BLOG_(\d+)_(.+)$/);
    if (!match) continue;
...
export function envGroupToBlogTarget(
  ...
  for (const key of REQUIRED_KEYS) {
    if (!group[key]) {
      errors.push(`Missing required field: XUN_BLOG_${blogNumber}_${key}`);
    }
  }
