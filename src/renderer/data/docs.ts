...
Xin is a lightweight desktop note-taking and journaling application for macOS. It's designed for a specific workflow: daily notes with tag-based organization and integrated blog publishing.
...
Xin supports multiple blogs. When you create a blog post, simply select which blog it should be published to.

### GitHub + Cloudflare Pages

Xin is built specifically for blogs hosted on GitHub and deployed via Cloudflare Pages. This opinionated approach means less configuration and a streamlined publishing experience.`,
  },
...
Xin provides several ways to quickly access daily notes.
...
Xin supports multiple blogs. Configure each one in Settings, and when you publish a post, select which blog it should go to.
...
Blog posts in Xin are written using a special block format within your notes.
...
Xin provides keyboard shortcuts for common navigation tasks.
